...
import {CustomAlert, DoneAlert, Header} from '../components';
...
const StudyView = () => {
  ...
  const {setAnswerChecked, currentQuestionIndex, setCurrentQuestionIndex} =
    ...
    const getData = async () => {
      try {
        let response = await axios.get(
          `http://${IPV4_ADDRESS}:${PORT}/api/v1/lesson/topic/${id}`,
          {
            headers: {
    ...
    try {
      if (currentQuestion && currentQuestion.lesson) {
        ...
        switch (type) {
          ...
          case 'Speaking':
            return (
              <SpeakingQuestion question={currentQuestion.lesson.question}
              />
            );
          default:
            return (
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.contentContainer}>
        <Text style={{color: COLORS.Black, fontSize: FONTSIZE.size_16, marginHorizontal: SPACING.space_20, alignSelf: 'flex-end'}}>Câu: {currentQuestionIndex+1}/{questionList.length}</Text>
